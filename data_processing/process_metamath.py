"""
Preprocess the MetaMathQA dataset for DiaBlo training.

Downloads from HuggingFace: meta-math/MetaMathQA
...
data = load_dataset("meta-math/MetaMathQA", cache_dir=data_cache)
train_data = data["train"].shuffle().map(generate_and_tokenize_prompt, num_proc=32)

save_path = os.path.join(args.output_dir, f"metamath/train_all_{cutoff_len}_OnlyOutput")
